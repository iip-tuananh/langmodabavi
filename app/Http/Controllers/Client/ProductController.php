...
class ProductController extends Controller
{
    ...
    public function detail_product($slug)
    {
        ...
        $data['productlq'] = Product::where(['status'=>1, 'cate_slug'=>$data['product']->cate_slug])->get(['id','name','images','discount','price','slug','cate_slug','type_slug','description']);
...
// return;
    ...
    public function filterProductCate(Request $request)
    {
        $id = $request->id;
        $cate = Category::findOrFail($id);
        $data['products'] = Product::where(['status'=>1, 'cate_slug'=>$cate->slug])->where('discountStatus', '>', 0)->limit(6)->get();
        $html = view('layouts.product.filter-cate-home', $data)->render();
